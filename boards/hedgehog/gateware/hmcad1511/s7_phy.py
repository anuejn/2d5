...
class HMCAD1511Phy(Elaboratable):
    ...
    def __init__(self):
        ...
        self.sensor_clk = ControlSignal()

    def elaborate(self, platform):
        m = Module()

        self.spi = m.submodules.spi = HMCAD1511SPI()
        m.submodules.clk_bit = ClockDebug("bit_clk")  # sync * 2
        m.submodules.clk_frame = ClockDebug("frame_clk")  # sync / 2
        m.submodules.clk_output = ClockDebug("sync")

        adc = platform.request("hmcad1511")
        m.d.comb += adc.reset.o.eq(self.reset)
        m.d.comb += adc.power_down.o.eq(self.power_down)
        m.d.comb += adc.clk.o.eq(ClockSignal())
...
class HMCAD1511Lane(Elaboratable):
    ...
    def elaborate(self, platform):
        ...
        bitslip_syncronizer = PulseSynchronizer(PERIPHERAL_DOMAIN, "frame_clk")
