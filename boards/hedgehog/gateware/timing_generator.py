...
class SlothTimingGenerator(Elaboratable):
    def __init__(self, sensor_digital):
        ...
        self.y_ctr = Signal(16)
        self.x_ctr = Signal(16)
    ...
    def elaborate(self, platform):
        ...
        m.d.comb += self.last.eq(
            (self.x_ctr == self.line_length-1) &
            (self.y_ctr == self.frame_lines-1)
        )

        with m.If(self.x_ctr < self.line_length-1):
            m.d.sync += self.x_ctr.eq(self.x_ctr + 1)
        with m.Else():
            m.d.sync += self.x_ctr.eq(0)
            with m.If(self.y_ctr < self.frame_lines-1):
        ...
        for pin_name, changes in TIMING.items():
            ...
            m.d.comb += getattr(self.sensor_digital, pin_name).o.eq(pin)
            m.submodules[pin_name] = PulseGenerator(self.x_ctr, pin, changes)

        for pin_name, changes in CLOCKS.items():
            enable = Signal(name=f"{pin_name}_enable")
            m.submodules[pin_name] = PulseGenerator(self.x_ctr, enable, changes)
            pin = Signal(name=pin_name)
            m.d.comb += getattr(self.sensor_digital, pin_name).o.eq(pin)
            m.d.comb += pin.eq(~self.x_ctr[0] & enable)

        for pin_name, (gating_y, gating_x) in Y_GATED.items():
            enable_y = Signal(name=f"{pin_name}_enable_y")
            m.submodules[f"{pin_name}_y"] = PulseGenerator(self.y_ctr, enable_y, gating_y)
            enable_x = Signal(name=f"{pin_name}_enable_x")
            m.submodules[f"{pin_name}_x"] = PulseGenerator(self.x_ctr, enable_x, gating_x)
            pin = Signal(name=pin_name)
            m.d.comb += getattr(self.sensor_digital, pin_name).o.eq(pin)
